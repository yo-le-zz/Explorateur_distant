...
class ServerManagerUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.context_menu = tk.Menu(self.lb, tearoff=0)
        self.context_menu.add_command(label="Ouvrir", command=self.connect_server) # Ajouté pour cohérence
        self.context_menu.add_command(label="Voir les infos", command=self.show_info)
        # --- NOUVEAU: Modifier les infos ---
        self.context_menu.add_command(label="Modifier infos", command=self.edit_server) 
        self.context_menu.add_command(label="Supprimer", command=self.delete_server)
    ...
    # --- NOUVELLE MÉTHODE: Modifier le serveur sélectionné ---
    def edit_server(self):
        cur = self.lb.curselection()
        if not cur:
            messagebox.showwarning("Sélection requise", "Veuillez sélectionner un serveur à modifier.", parent=self)
            return

        idx = cur[0]
        entry_to_edit = self.entries[idx]
        
        # Import local pour éviter les références circulaires
        from config import prompt_new_server, save_entries 
        
        # NOTE: prompt_new_server doit être adapté dans config.py pour accepter entry_to_edit
        result = prompt_new_server(self, entry_to_edit=entry_to_edit) 
        ...
        self.refresh_list()
        messagebox.showinfo("Succès", "Serveur modifié avec succès.", parent=self)
    
    # --- MÉTHODE EXISTANTE: Afficher le menu contextuel (maintenue) ---
    def show_context_menu(self, event):
        # Identifier l'élément cliqué
        try:
            # Récupérer l'index de l'élément à la position du clic
            index = self.lb.nearest(event.y)
            
            # Si le clic est sur un élément, le sélectionner et afficher le menu
            if index is not None:
                self.lb.selection_clear(0, tk.END) # Désélectionner tout
                self.lb.selection_set(index)       # Sélectionner l'élément
                self.lb.activate(index)            # Activer l'élément pour focus
    ...
    def show_info(self):
        ...
        # Clés: "user_serveur", "user_host", "user_port", "user_start_path", "key_path", "password"
        
        info_text = f"Nom d'utilisateur: {entry.get('user_serveur', 'N/A')}\n"
        info_text += f"Hôte: {entry.get('user_host', 'N/A')}\n"
        info_text += f"Port: {entry.get('user_port', '22')}\n"
        
        # Authentification
        key_path = entry.get('key_path')
        auth_type = "Clé SSH" if key_path else "Mot de passe"
        
        info_text += f"Authentification: {auth_type}\n"
        
        if key_path:
            info_text += f"Chemin de la clé: {key_path}\n"
        
        info_text += f"Chemin de démarrage: {entry.get('user_start_path', '/')}"
        
        messagebox.showinfo(
            f"Infos Serveur - {entry.get('user_serveur', 'N/A')}@{entry.get('user_host', 'N/A')}", 
            info_text, 
            parent=self
        )
    ...
    def refresh_list(self):
        ...
        for e in self.entries:
            # On affiche une combinaison du nom d'utilisateur et de l'hôte
